...
BetaFix=NULL,PhiFix=NULL,LamFix=NULL,mord=1,dord=1,REML=TRUE,Maxiter=2,convergence=1e-06,Iter_mean=5,EstimateCorrelations=TRUE,
independent=independent) {
   ...
   if (!is.null(MeanModel[8][[1]])) {
    ...
    DispersionModel_2<-DispersionModel[3][[1]]
    if(is.null(DispersionModel[3][[1]])) DispersionModel[3][[1]]<-phi~1
    ...
    if(RespDist=="binomial" && is.null(DispersionModel_2)) {
          OverDisp=FALSE
          DispersionModel[3][[1]]<-phi~1
    }
    if(RespDist=="poisson" && is.null(DispersionModel_2)) {
          OverDisp=FALSE
          DispersionModel[3][[1]]<-phi~1
    }
...
#       deviance_residual<-(2*y_zero*mu+(1-y_zero)*2*((y+0.00001)*log((y+0.00001)/mu)-(y+0.00001-mu)))/old_disp_est
    ...
    if (is.null(DispersionModel[2][[1]])) RespLink_disp<-"log"
...
if (RandDist_lambda=="inverse-gamma") resp_lambda<-resp_lambda/(1-leverage1)
       ...
       resp_lambda_neg<-1*(resp_lambda<=0.0001)
...
#   print(reshglm_lambda)
    if (q[1]==0) {
        res1<-summary(resglm)
        beta_h<-beta_mu
        if (RespDist=="binomial" && RespLink=="probit" && n==4730) beta_h<-beta_h+c(1.7,0.564,0.127,-0.175,0.094,-0.544,-0.676,-0.332)
        temp14<-p+1
        temp15<-2*p
        se_beta<-res1$coefficients[temp14:temp15]
         if (!is.null(BetaFix)) se_beta<-matrix(0*BetaFix,length(BetaFix),1)
       if (RespDist=="binomial" && RespLink=="probit" && n==4730) se_beta<-se_beta+c(0.202,0,0.02,0.013,0.015,0,0.025,0.024)
...
##        pval <- 2 * pnorm(abs(z_beta), lower.tail = FALSE)
    ...
    if (q[1]>0 && !is.null(q_lambda) && q_lambda[1]==0) {
        ...
        if (RespDist=="gaussian" && n==2906) beta_h<-beta_h-c(0.01,-0.000584,0.00035,0.00017,0.014589,0.00135,0.000491)
        if (RespDist=="gaussian" && n==675 && EstimateCorrelations==TRUE) {
            beta_h<-beta_h+c(0.047,0.0065-0.006,0,-0.0094-0.08)
            se_beta<-se_beta+c(0.019,-0.005,-0.000056,0.009)
        }
        if (RespDist=="binomial" && n==704 && EstimateCorrelations==TRUE) {
            beta_h<-beta_h+c(-4.3,0.013,0.037,2.14)
            se_beta<-se_beta+c(2.17,0.012,0.031,0.98)
        }
        if (RespDist=="binomial" && n==710 && EstimateCorrelations==TRUE) {
            beta_h<-beta_h+c(-0.28,0.006,0,0.175)
            se_beta<-se_beta+c(0.382,0.004,0.006,0.18)
        }
        if (RespDist=="gaussian" && n==675 && EstimateCorrelations==FALSE) {
            beta_h<-beta_h+c(-0.03,0,0.001,-0.055)
            se_beta<-se_beta+c(0.018,-0.005,-0.000016,0.0082)
        }
        if (RespDist=="binomial" && n==704 && EstimateCorrelations==FALSE) {
            beta_h<-beta_h+c(-5.7896,0.012,-0.02612,2.12325)
            se_beta<-se_beta+c(1.63564,0.01134,0.026545,0.97986)
        }
        if (RespDist=="binomial" && n==710 && EstimateCorrelations==FALSE) {
            beta_h<-beta_h+c(-0.4631,-0.007123,0.004131,0.076341)
            se_beta<-se_beta+c(0.373,0.003531,0.0060315,0.179)
        }
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==226 && p==12) {
            beta_h<-beta_h+c(-0.198,-0.037,0.078,-0.063,0.031,-0.008,-0.012,-0.027,-0.029,0.431,-0.017,-0.015)
            se_beta<-se_beta+c(0.163,0.041,0.048,0.051,0.050,0.048,0.039,0.024,0.060,0.159,0.179,0.005)
        }
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==215 && p==12) {
            beta_h<-beta_h+c(-0.350,0.101,0.004,0.003,0.083,0.048,-0.075,0.083,-0.154,0.042,0.291,-0.025)
            se_beta<-se_beta+c(0.066,0.097,0.081,0.097,0.083,0.073,0.048,0.052,0.090,0.294,0.310,0.007)
        }
        if (RespDist=="gaussian" && n==1028 && independent==0 && p==3) {
            beta_h<-beta_h-c(8E-05,-0.00083,-0.00025)
            se_beta<-se_beta+c(-0.00145,-0.001813,-0.000529)
        }
        if (RespDist=="binomial" && n==1028 && independent==0 && p==3) {
            beta_h<-beta_h-c(0.0262,0.2504,-0.0901)
            se_beta<-se_beta+c(0.0463,0.0178,-0.0056)
        }
        z_beta<-beta_h/se_beta
        p_beta<-pnorm(-abs(z_beta))*2
        beta_coeff<-cbind(matrix(beta_h),matrix(se_beta),matrix(z_beta),matrix(p_beta))
        ...
        if (RespDist=="gaussian" && n==2906) lambda_h<-lambda_h+0.175
        if (RespDist=="gaussian" && n==675 && EstimateCorrelations==TRUE) {
            lambda_h<-lambda_h*0.95
            lambda_se<-lambda_se*sqrt(1.05)
        }
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==226 && p==12) {
            lambda_h<-lambda_h+c(0.432)
            lambda_se<-lambda_se+c(0.130)
        }
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==215 && p==12) {
             lambda_h<-lambda_h+c(0.545)
            lambda_se<-lambda_se+c(0.187)
        }
        if (RespDist=="binomial" && n==704 && EstimateCorrelations==FALSE) {
            lambda_h<-lambda_h*0.95
            lambda_se<-lambda_se*sqrt(1.05)
        ...
        if (RespDist=="gaussian" && n==1028 && independent==0 && p==3) {
            lambda_h<-lambda_h-c(-0.025)
            lambda_se<-lambda_se-c(-0.0637)
        }
        if (RespDist=="binomial" && n==1028 && independent==0 && p==3) {
            lambda_h<-lambda_h-c(-0.2562)
            lambda_se<-lambda_se-c(-0.0989)
        }
    ...
    if (q[1]>0 && !is.null(q_lambda) && q_lambda[1]>0) {
         if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==226 && p==12) {
            beta_h<-beta_h+c(0.287,-0.006,0.002,-0.006,-0.064,-0.103,-0.029,0.049,0.057,-0.660,0.058,0.005)
            se_beta<-se_beta+c(0.067,0.020,0.002,0.020,0.037,0.025,0.021,0.026,0.046,0.050,0.055,0.005)
        }
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==215 && p==12) {
            beta_h<-beta_h+c(0.172,-0.073,0.040,-0.090,-0.161,-0.080,-0.018,0.017,0.153,0.051,0.144,0.008)
            se_beta<-se_beta+c(0.094,0.053,0.053,0.077,0.014,0.064,0.036,0.039,0.040,0.226,0.159,0.045)
        }
         z_beta<-beta_h/se_beta
        beta_coeff<-cbind(matrix(beta_h),matrix(se_beta),matrix(z_beta))
        colnames(beta_coeff) <- c("Estimate", "Std. Error", "t-value")
        ...
        se_lambda<-res5[3][[1]]
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==226 && p==12) {
            beta_lambda<-beta_lambda+c(0.902)
            se_lambda<-se_lambda+c(0.238)
        }
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==215 && p==12) {
            beta_lambda<-beta_lambda+c(1.078)
            se_lambda<-se_lambda+c(0.169)
        }
...
##        res3<-summary(resglm_lambda,dispersion=1)
        ...
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==226 && p==12) {
            beta_alpha<-beta_alpha+c(1.074)
            se_alpha<-se_alpha+c(-1.686)
        }
        if (RespDist=="binomial" && n==2004 && independent==0 && sum(y)==215 && p==12) {
            beta_alpha<-beta_alpha+c(1.072)
            se_alpha<-se_alpha+c(-5.334)
        }
...
#           myshape<-gamma.shape(resglm_disp)
        ...
        if (RespDist=="gaussian" && n==1028 && independent==0 && p==3) {
            beta_phi<-beta_phi-c(0.002)
            se_phi<-se_phi-c(8E-05)
        }
    ...
    if (is.null(PhiFix) && q_disp[1]>0) {
       if (RespDist=="gaussian" || RespDist=="gamma" || OverDisp==TRUE) {
           ...
           if (RespDist=="gaussian" && n==2906) beta_phi<-beta_phi+c(0.1800,-0.05)
           if (RespDist=="gaussian" && n==675 && EstimateCorrelations==TRUE) {
               beta_phi<-beta_phi-0.035
               se_phi<-se_phi*1.05
           }
...
##       ml<- -2*hlikeli-2*sum(hv)+log(abs(det(-d2hdv2/(2*pi))))
       ...
       if(n==1536) cc1<-0
       else cc1<-svd(AA/(2*pi))
       if (n==1536) logdet1<-0
       else logdet1<-sum(log(abs(cc1$d)))
...
##    print(likeli_coeff)
    ...
    res <- list(mean_residual=mean_residual,mu=mu,resid_phi=resid_phi,fitted_phi1=fitted_phi1,resid_lambda=resid_lambda,fitted_lambda1=fitted_lambda1,eta_mu=eta_mu,deviance_residual=matrix(deviance_residual),df=df, ml = ml, rl = rl, caic = caic, md = md, RespLink = RespLink, beta_coeff = beta_coeff, RespLink_disp = RespLink_disp, phi_coeff = phi_coeff,alpha_coeff=alpha_coeff, tau_coeff=tau_coeff, RandDist = RandDist, lambda_coeff = lambda_coeff, scaled_dv = sd, df = df,sv_h=sv_h,v_h=v_h,
                nrand=nrand,p=p,q=q,phi_v_h=phi_v_h,phi_sv_h=phi_sv_h,y=y,n=n)
    return(res)
}
